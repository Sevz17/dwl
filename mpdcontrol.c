...
void mpd_volume(const Arg *arg) {
    ...
    if (arg->i) {
        mpd_run_change_volume(conn, arg->i);
    }
...
void mpdcontrol(){
    ...
    if(state == MPD_STATE_STOP || state == MPD_STATE_PAUSE){
        ...
        errcode = regcomp(&expr, "^[[:alnum:]]+://", REG_EXTENDED|REG_NOSUB);
        if(errcode != 0){
            err = get_regerror(errcode, &expr);
        ...
        matchcode = regexec(&expr, filename, 0, NULL, 0);
        ...
        if(matchcode == 0){
            if(strstr(filename, "file://") == filename){ //match just at the start of the filename
                ...
                mpd_run_pause(conn, state == MPD_STATE_PLAY);
            }
            else{
                mpd_run_stop(conn);
            }
            ...
            err = get_regerror(matchcode, &expr);
